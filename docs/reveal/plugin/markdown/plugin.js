...
const Plugin = () => {
  ...
  function stripFrontMatterData(markdown, options) {
    markdown.match(/---(.*?)---(.*)/is);
    const frontMatter = RegExp.$1.split(options.separator);
    ...
    const frontMatter = stripFrontMatterData(markdown, options);
    markdown = markdown.replace(/---.*?---/is, "");
