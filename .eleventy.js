...
`});
  ...
  eleventyConfig.addShortcode('question', (question, tagline, props) => {
  ...
  eleventyConfig.addShortcode('simpleInterlude', (title, text, transition) => {
    const titleHtml = title ? `<h1 class="title">${insertMarkup(title)}</h1>` : '';
    const textHtml = text ? md.render(text) : '';
    const dataTransition = transition ? `data-transition="${transition}"` : '';
    return `<section data-slide-shortcode-class="simple-text" class="simple is-interlude" ${dataTransition}>
    <div>${titleHtml}${textHtml}</div></section>`;
  });

  eleventyConfig.addShortcode('shout', (title, author, source, info, transition) => {
    const titleHtml = title ? `<h1 class="title">${insertMarkup(title)}</h1>` : '';
    const authorHtml = author ? `<cite>${insertMarkup(author)}</cite>` : '';
    const sourceHtml = source ? `<div class="src">${insertMarkup(source)}</div>` : '';
    const infoHtml = info ? `<p class="info is-passive">${insertMarkup(info)}</p>` : '';
    const dataTransition = transition ? `data-transition="${transition}"` : '';

    return `
    <section data-slide-shortcode-class="shout" class="shout" ${dataTransition}>
      <div>
        <blockquote class="has-whitener">${titleHtml}${authorHtml}${sourceHtml}</blockquote>
        ${infoHtml}
      </div>
    </section>
  `;
